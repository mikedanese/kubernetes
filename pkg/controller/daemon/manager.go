...
type DaemonManager struct {
	...
	dcStore cache.StoreToDaemonLister
	...
	dm.dcStore.Store, dm.dcController = framework.NewInformer(
		&cache.ListWatch{
			ListFunc: func() (runtime.Object, error) {
				return dm.kubeClient.Daemons(api.NamespaceAll).List(labels.Everything())
			},
...
func (dm *DaemonManager) updateDaemonStatus(dc *expapi.Daemon) {
	...
	err = storeDaemonStatus(dm.kubeClient.Daemons(dc.Namespace), dc, desiredNumberScheduled, currentNumberScheduled, numberMisscheduled)
	if err != nil {
