...
func (x *KubeletConfiguration) CodecEncodeSelf(e *codec1978.Encoder) {
	...
	if x == nil {
		...
		if false {
			...
			yyq2[75] = x.NodeIP != ""
			yyq2[79] = x.Kind != ""
			yyq2[80] = x.APIVersion != ""
			var yynn2 int
			if yyr2 || yy2arr2 {
				r.EncodeArrayStart(81)
			} else {
				yynn2 = 68
				for _, b := range yyq2 {
					if b {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "streamingConnectionIdleTimeout":
			if r.TryDecodeAsNil() {
				...
				yyv48 := &x.StreamingConnectionIdleTimeout
				yym49 := z.DecBinary()
				_ = yym49
				if false {
		...
		case "nodeStatusUpdateFrequency":
			if r.TryDecodeAsNil() {
				...
				yyv50 := &x.NodeStatusUpdateFrequency
				yym51 := z.DecBinary()
				_ = yym51
				if false {
...
func (x *KubeletConfiguration) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv124 := &x.HostIPCSources
		yym125 := z.DecBinary()
		_ = yym125
		if false {
		} else {
			z.F.DecSliceStringX(yyv124, false, d)
		}
	}
	yyj98++
	if yyhl98 {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
			...
			z.F.DecMapStringStringX(yyv187, false, d)
		}
	}
	yyj98++
	if yyhl98 {
