		...
		if false {
			...
			if yyr102 || yy2arr102 {
				...
				if false {
					...
					r.EncodeString(codecSelferC_UTF81234, string(x.PidFilePath))
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
				r.EncodeString(codecSelferC_UTF81234, string("pidFilePath"))
				z.EncSendContainerState(codecSelfer_containerMapValue1234)
				yym270 := z.EncBinary()
				_ = yym270
				if false {
...
func (x *KubeletConfiguration) CodecDecodeSelf(d *codec1978.Decoder) {
	...
	if false {
		...
		yyct334 := r.ContainerType()
		if yyct334 == codecSelferValueTypeMap1234 {
			yyl334 := r.ReadMapStart()
			if yyl334 == 0 {
				z.DecSendContainerState(codecSelfer_containerMapEnd1234)
			} else {
				x.codecDecodeSelfFromMap(yyl334, d)
			}
		} else if yyct334 == codecSelferValueTypeArray1234 {
			yyl334 := r.ReadArrayStart()
			if yyl334 == 0 {
				z.DecSendContainerState(codecSelfer_containerArrayEnd1234)
			} else {
				x.codecDecodeSelfFromArray(yyl334, d)
			}
		} else {
			panic(codecSelferOnlyMapOrArrayEncodeToStructErr1234)
		}
...
func (x *KubeletConfiguration) codecDecodeSelfFromMap(l int, d *codec1978.Decoder) {
	...
	z, r := codec1978.GenHelperDecoder(d)
	_, _, _ = h, z, r
	var yys335Slc = z.DecScratchBuffer() // default slice to decode into
	_ = yys335Slc
	...
	for yyj335 := 0; ; yyj335++ {
		...
		z.DecSendContainerState(codecSelfer_containerMapKey1234)
		yys335Slc = r.DecodeBytes(yys335Slc, true, true)
		yys335 := string(yys335Slc)
		z.DecSendContainerState(codecSelfer_containerMapValue1234)
		switch yys335 {
		case "config":
			if r.TryDecodeAsNil() {
		...
		case "streamingConnectionIdleTimeout":
			if r.TryDecodeAsNil() {
				...
				yyv377 := &x.StreamingConnectionIdleTimeout
				yym378 := z.DecBinary()
				_ = yym378
				if false {
		...
		case "nodeStatusUpdateFrequency":
			if r.TryDecodeAsNil() {
				...
				yyv379 := &x.NodeStatusUpdateFrequency
				yym380 := z.DecBinary()
				_ = yym380
				if false {
		...
		case "pidFilePath":
			if r.TryDecodeAsNil() {
				...
				x.PidFilePath = string(r.DecodeString())
			}
		case "rktStage1Image":
			if r.TryDecodeAsNil() {
				x.RktStage1Image = ""
		...
		case "outOfDiskTransitionFrequency":
			if r.TryDecodeAsNil() {
				...
				yyv410 := &x.OutOfDiskTransitionFrequency
				yym411 := z.DecBinary()
				_ = yym411
				if false {
		...
		default:
			z.DecStructFieldNotFound(-1, yys335)
		} // end switch yys335
	} // end for yyj335
	z.DecSendContainerState(codecSelfer_containerMapEnd1234)
}
...
func (x *KubeletConfiguration) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv422 := &x.HTTPCheckFrequency
		yym423 := z.DecBinary()
		_ = yym423
		if false {
		} else if z.HasExtensions() && z.DecExt(yyv422) {
		} else if !yym423 && z.IsJSONHandle() {
			z.DecJSONUnmarshal(yyv422)
		} else {
			z.DecFallback(yyv422, false)
		}
	}
	yyj416++
	if yyhl416 {
	...
	if r.TryDecodeAsNil() {
		...
		yyv458 := &x.StreamingConnectionIdleTimeout
		yym459 := z.DecBinary()
		_ = yym459
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv460 := &x.NodeStatusUpdateFrequency
		yym461 := z.DecBinary()
		_ = yym461
		if false {
