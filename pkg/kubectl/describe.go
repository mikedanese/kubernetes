...
func describePod(pod *api.Pod, rcs []api.ReplicationController, daemons []api.Daemon, events *api.EventList) (string, error) {
	return tabbedString(func(out io.Writer) error {
		fmt.Fprintf(out, "Name:\t%s\n", pod.Name)
		fmt.Fprintf(out, "Namespace:\t%s\n", pod.Namespace)
...
func describeDaemon(daemon *api.Daemon, events *api.EventList, running, waiting, succeeded, failed int) (string, error) {
	return tabbedString(func(out io.Writer) error {
		fmt.Fprintf(out, "Name:\t%s\n", daemon.Name)
...
func getDaemonsForLabels(c client.DaemonInterface, labelsToMatch labels.Labels) ([]api.Daemon, error) {
	...
	var matchingDaemons []api.Daemon
...
func printDaemonsByLabels(matchingDaemons []api.Daemon) string {
