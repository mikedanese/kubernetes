...
def run():
  ...
  if grains["roles"][0] == "kubernetes-master":
    if grains["cloud"] in ["aws", "gce", "vagrant", "vsphere"]:
      ...
      config["registerSchedulable"] = False
      config["reconcileCIDR"] = False
  ...
  standalone = grains.get("api_servers", "") == ""
  if grains["roles"][0] == "kubernetes-master":
    if "cbr-cidr" in grains:
      config["podCIDR"] = grains["cbr-cidr"]
    else if standalone and grains.get('network_provider', '').lower() == 'kubenet':
      config["podCIDR"] = "10.76.0.0/16"
  ...
  if "hairpin_mode" in pillar:
    config["hairpinMode"] = pillar["hairpin_mode"] == "true"

  if "enable_custom_metrics" in pillar:
    config["enableCustomMetrics"] = pillar["enable_custom_metrics"]
